...
class CimCLR_RBC(nn.Module):
    ...
    def forward(self, y1, y2, b1, b2): # 4つの画像を入力
        z1 = self.projector(self.backbone(y1) - self.backbone(b1)) # 背景の特徴量を引いたものをzとする
        z2 = self.projector(self.backbone(y2) - self.backbone(b2)) # 背景の特徴量を引いたものをzとする
...
class LinearHead(nn.Module): # 精度評価等に使う、一旦無視
